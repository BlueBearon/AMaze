...
public interface P5Panel21 
{
	// TODO FOR P5:
	    ...
	    public static int getWallColor(int distance, int cc, int extensionX) 
	    {
	        ...
	        switch (((d >> 3) ^ cc) % 6) {
	        case 0:
	        	return (MazePanel.getColorfromRGB(rgbValue, RGB_DEF, RGB_DEF));
	        case 1:
	        	return(MazePanel.getColorfromRGB(RGB_DEF, RGB_DEF_GREEN, RGB_DEF));
	        case 2:
	        	return (MazePanel.getColorfromRGB(RGB_DEF, RGB_DEF, rgbValue));
	        case 3:
	        	return (MazePanel.getColorfromRGB(rgbValue, RGB_DEF_GREEN, RGB_DEF));
	        case 4:
	        	return (MazePanel.getColorfromRGB(RGB_DEF, RGB_DEF_GREEN, rgbValue));
	        case 5:
	        	return (MazePanel.getColorfromRGB(rgbValue, RGB_DEF, rgbValue));
	        default:
	        	return (MazePanel.getColorfromRGB(RGB_DEF, RGB_DEF, RGB_DEF));
	        }
		 ...
		 * 
		 * This method was adjusted to include viewWidth and viewHeight
		 * as the method in FirstPersonView uses these variables to define size
		 * of the rectangles drawn.  Therefore, in order to avoid making drastic changes,
		 * it would be better to just get these variables from FirstPersonView.
		 * 
		 * @param percentToExit gives the distance to exit
		 * @param viewWidth  used to calculate dimensions of rectangle
		 * @param viewHeight used to calculate dimensions of rectangle
		 */
