...
public class Map {
	...
	public void draw(MazePanel panel, int x, int y, int angle, int walkStep,
        ...
        drawMap(x, y, walkStep, viewDX, viewDY, showMaze, showSolution, panel) ;
        drawCurrentLocation(viewDX, viewDY, panel) ;
	}
	 ...
	 * @param panel TODO
	 */
	private void drawMap(int px, int py, int walkStep, int viewDX, 
			int viewDY, boolean showMaze, boolean showSolution, MazePanel panel) {
		...
		for (int y = minY; y <= maxY; y++)
			for (int x = minX; x <= maxX; x++) {
				...
				if (x < mazeWidth)
					drawHorizontalLine(showMaze, x, y, startX, startY, panel);
				if (y < mazeHeight)
					drawVerticalLine(showMaze, x, y, startX, startY, panel);
			}
		
		if (showSolution) {
			drawSolution(offsetX, offsetY, px, py, panel) ;
		}
	}
	 ...
	 * @param panel TODO
	 */
	private void drawVerticalLine(boolean showMaze, int x, int y, int startX, int startY, MazePanel panel) {
		if (hasAVerticalWall(x, y) && 
				(seenWalls.hasWall(x, y, CardinalDirection.West) || showMaze)) {
			panel.setColor(seenWalls.hasWall(x, y, CardinalDirection.West) ? -1 : -8355712);//white or gray
	 ...
	 * @param panel TODO
	 */
	private void drawHorizontalLine(boolean showMaze, int x, int y, int startX, int startY,
			MazePanel panel) {
		if (hasAHorizontalWall(x, y) && (seenWalls.hasWall(x,y, CardinalDirection.North) || showMaze) ) {
			panel.setColor(seenWalls.hasWall(x, y, CardinalDirection.North) ? -1 : -8355712);//white or gray
	...
	private void drawCurrentLocation(int viewDX, int viewDY, MazePanel panel) {
		...
		drawArrow(viewDX, viewDY, centerX, centerY, panel);
	}
	 ...
	 * @param panel TODO
	 */
	private void drawArrow(int viewDX, int viewDY, final int startX, 
			final int startY, MazePanel panel) {
	 ...
	 * @param panel TODO
	 */
	private void drawSolution(int offsetX, int offsetY, int px, int py, MazePanel panel) {
